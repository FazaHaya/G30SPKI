...
<nav class="bg-[#3e0d02] text-white py-4 shadow">
  <div class="container mx-auto px-6 flex justify-between items-center">
    <a href="{{ route('home') }}" class="text-left leading-tight">
      <h1 class="text-xl font-bold">G30S</h1>
      <span class="text-sm font-semibold tracking-wide">Chronicles.<sup>®</sup></span>
    </a>
    <ul class="hidden md:flex space-x-8 text-md font-light">
      <li><a href="#tokoh" class="hover:text-red-300">History</a></li>
      <li><a href="#quiz" class="hover:text-red-300">Quiz</a></li>
      <li><a href="#contact" class="hover:text-red-300">Contact</a></li>
    </ul>
    <a href="{{ route('chatbox') }}" class="bg-white text-[#3e0d02] font-semibold text-sm py-2 px-4 rounded-md shadow-md hover:bg-gray-100 transition">
      Ask AI
  </a>
  </div>
</nav>
...
<section class="bg-[#3e0d02] text-white py-20">
  <div class="container w-[80%] mx-auto px-4 flex flex-col lg:flex-row items-center">
    <div class="lg:w-1/2 space-y-6">
      ...
      <button onclick="scrollToSection('tokoh')" class="bg-white text-[#3e0d02] px-6 py-2 rounded-md font-semibold hover:bg-gray-100 transition">
        Know More
      </button>
    </div>
...
<section class="bg-white py-16">
  <div class="container w-[80%] mx-auto px-6 grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
    ...
    <div>
      ...
      <button onclick="scrollToSection('artikel')" class="mt-6 bg-black text-white text-sm font-semibold px-6 py-3 rounded-md shadow hover:bg-gray-800 transition">
        Know More
      </button>
    </div>
  </div>
...
<section id="quiz" class="py-20 bg-white">
  <div class="container w-[80%] mx-auto px-4 flex flex-col lg:flex-row items-center justify-between gap-12">
...
<section id="artikel" class="py-20 bg-gray-50">
  <div class="container w-[80%] mx-auto px-6">
...
<div class="flex justify-center items-center">
  <div class="container w-[80%] flex flex-col md:flex-row items-center justify-center gap-20  ">
...
<!-- Contact Section -->
<section id="contact" class="bg-[#3e0d02] text-white py-16 px-4">
<div class="max-w-6xl mx-auto flex flex-col lg:flex-row gap-12">
  ...
  <div class="lg:w-1/2 bg-white text-black rounded-xl p-8 shadow-lg">
    <h2 class="text-2xl font-bold mb-6 text-center">Tell Us Your Opinion</h2>
    <form class="space-y-4" id="komenForm">
      <div class="flex flex-col md:flex-row gap-4">
        <input type="text" id="nama" placeholder="Name" required
          class="w-full px-4 py-2 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-red-700" />
      </div>
      <textarea rows="4" id="komen" placeholder="Message" required
        class="w-full px-4 py-2 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-red-700"></textarea>
      <button type="submit"
        class="w-full bg-red-800 text-white font-semibold py-2 rounded-lg hover:bg-red-900 transition duration-200">
        Submit
      </button>
    </form>
  </div>
...
<!-- Komentar Card Section -->
<div class="max-w-6xl mx-auto mt-16">
  <div class="bg-white text-black rounded-xl p-8 shadow-lg w-full min-h-[400px]">
    <h2 class="text-2xl font-bold mb-4">Komentar</h2>
    <div id="komenList" class="space-y-4">
...
<!-- Pastikan tag ini ada di <head> jika pakai Laravel -->
...
async function fetchKomentar() {
  try {
    const res = await fetch('/api/getKomen');
    const data = await res.json();

    const list = document.getElementById('komenList');
    list.innerHTML = '';
    data.forEach(k => {
      const div = document.createElement('div');
      div.className = 'bg-gray-100 p-4 rounded shadow text-black';
      div.innerHTML = `<strong>${k.nama}</strong><p class="mt-1 text-gray-700">${k.komen}</p>`;
...
document.getElementById('komenForm').addEventListener('submit', async e => {
  e.preventDefault();
  const nama = document.getElementById('nama').value.trim();
  const komen = document.getElementById('komen').value.trim();

  if (!nama || !komen) return;

  try {
    await fetch('/api/addKomen', {
      method: 'POST',
      headers: {
